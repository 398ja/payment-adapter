...
class HttpMintWebhookForwarderTest {
     ...
     */
    @Test
    void notifyPaymentReceived_shouldSendTheCurrentTimeAsTheTimestamp() {
        java.time.Instant pinned = java.time.Instant.ofEpochSecond(1_700_000_000L);
        ReflectionTestUtils.setField(forwarder, "webhookSecret", "mysecret");
        HttpMintWebhookForwarder pinnedForwarder =
                new HttpMintWebhookForwarder(java.time.Clock.fixed(pinned, java.time.ZoneOffset.UTC));
        copyConfigurationTo(pinnedForwarder);
        stubFor(post(urlEqualTo("/webhook/payment")).willReturn(aResponse().withStatus(200)));

        pinnedForwarder.notifyPaymentReceived(
                PaymentNotification.forBolt11("quote123", 1000, "preimage456"));
    ...
    private void copyConfigurationTo(HttpMintWebhookForwarder target) {
        for (String field : new String[] {"enabled", "mintWebhookUrl", "webhookSecret",
